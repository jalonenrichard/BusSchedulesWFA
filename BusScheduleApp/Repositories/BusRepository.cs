    ...
    class BusRepository : DatabaseConnector, IRepository<Bus>
    {
        public List<Bus> GetAllBusSchedules()
        {
            ...
            while (reader.Read())
            {
                Bus bus = new Bus
                {
                    ...
                    DepartingTime = reader["departing_time"].ToString(),
        ...
        public void Delete(Bus bus)
        {
            string sql =
                ...
                "where bus_number = @param1 " +
                "and departing_station = @param2 " +
                "and destination_station = @param3 " +
                "and departing_time = @param4 " +
                "and arrival_time = @param5";
            OpenBusDbConnection();
            SQLiteCommand command = new SQLiteCommand(sql, MDbConnection)
            {
                CommandText = sql, CommandType = CommandType.Text
